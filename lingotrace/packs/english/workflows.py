"""English language pack workflows."""
...
from lingotrace.packs.english.validators import validate_review_materials, validate_review_rollover
...
def source_notes(
    ...
    *,
    ...
    errors, read_files = _target_context_errors(root, "source_notes")
    if errors:
        return _workflow_report("source_notes-workflow", mode, errors=errors, read_files=read_files)
            ...
            planned_writes=[
                {
                    ...
                    "reason": "target Vault has readable English review material",
    ...
    return _preview_report(
        ...
        errors=[
            Finding(
                ...
                message="Target Vault has no readable English review material for preview.",
            )
        ],
        read_files=read_files,
        )
    ...
    *,
    ...
    for card_path, fields in _cards_for_roles(root, paths, ROLLOVER_ROLES):
        ...
        if next_stage == "mastered" and fields.get("item_type") == "vocab":
            base_mutation = _en_base_vocab_sink_mutation(root, paths, card_path, fields)
...
def _missing_vault_root(capability_id: str) -> CommandReport:
    return _workflow_error(
        ...
        "preview",
        "missing_vault_root",
        "vault_root is required before running an English pack workflow.",
    )
...
def _en_base_vocab_sink_mutation(
    root: Path,
    paths: dict[str, str],
    focus_path: Path,
    ...
    if isinstance(base_match, Finding):
        return base_match
    stable_fields = _en_base_vocab_fields_from_focus(focus_fields, title)
    if base_match is not None:
...
def _en_base_vocab_fields_from_focus(focus_fields: dict[str, str], title: str) -> dict[str, Any]:
    fields: dict[str, Any] = {
        "track": "base_vocab",
        "item_type": "vocab",
        "status": "promoted",
    ...
    for key in _EN_STABLE_BASE_VOCAB_KEYS:
